...
macro(libbeauty_lapack_check)
  ...
  _find_header_file_in_dirs(LIBBEAUTY_CBLAS_H_PATH_${_lapack_impl} "${LAPACK_CBLAS_H}" "${LAPACK_INCLUDE_DIR}")
  _find_header_file_in_dirs(LIBBEAUTY_LAPACKE_H_PATH_${_lapack_impl} "${LAPACK_LAPACKE_H}" "${LAPACK_INCLUDE_DIR}")
  if(NOT LIBBEAUTY_CBLAS_H_PATH_${_lapack_impl} OR NOT LIBBEAUTY_LAPACKE_H_PATH_${_lapack_impl})
    message(WARNING "LAPACK(${LAPACK_IMPL}): CBLAS/LAPACK headers are not found in '${LAPACK_INCLUDE_DIR}'")
    unset(LAPACK_LIBRARIES)
  else()
    # adding proxy libbeauty_lapack.h header
    set(CBLAS_H_PROXY_PATH ${CMAKE_BINARY_DIR}/libbeauty_lapack.h)
    if((APPLE OR LIBBEAUTY_SKIP_LAPACK_EXTERN_C) AND NOT LIBBEAUTY_FORCE_LAPACK_EXTERN_C)
    ...
    set(_lapack_include_str "${_lapack_include_str_extern_C}\#include \"${LIBBEAUTY_CBLAS_H_PATH_${_lapack_impl}}\"")
    if(NOT "${LIBBEAUTY_CBLAS_H_PATH_${_lapack_impl}}" STREQUAL "${LIBBEAUTY_LAPACKE_H_PATH_${_lapack_impl}}")
      set(_lapack_include_str "${_lapack_include_str}\n#include \"${LIBBEAUTY_LAPACKE_H_PATH_${_lapack_impl}}\"")
    endif()
    ...
    try_compile(__VALID_LAPACK
        ...
        "${libbeauty_SOURCE_DIR}/cmake/checks/lapack_check.cpp"
    ...
    if(NOT __VALID_LAPACK)
      ...
      libbeauty_include_directories(${LAPACK_INCLUDE_DIR})
      set(HAVE_LAPACK 1)
    endif()
  endif()
endmacro()
...
if(WITH_LAPACK)
  libbeauty_update(LAPACK_IMPL "Unknown")
  if(NOT LIBBEAUTY_LAPACK_FIND_PACKAGE_ONLY)
    if(NOT LAPACK_LIBRARIES AND NOT LIBBEAUTY_LAPACK_DISABLE_MKL)
      include(cmake/libbeautyFindMKL.cmake)
      if(HAVE_MKL)
        ...
        set(LAPACK_IMPL         "MKL")
        libbeauty_lapack_check()
      endif()
    endif()
    if(NOT LAPACK_LIBRARIES)
      include(cmake/libbeautyFindOpenBLAS.cmake)
      if(OpenBLAS_FOUND)
    ...
    if(NOT LAPACK_LIBRARIES AND UNIX)
      include(cmake/libbeautyFindAtlas.cmake)
      if(ATLAS_FOUND)
  ...
  if(NOT LAPACK_LIBRARIES)
    if(WIN32 AND NOT LIBBEAUTY_LAPACK_SHARED_LIBS)
      set(BLA_STATIC 1)
    endif()
    find_package(LAPACK)
    if(LAPACK_FOUND)
      ...
      if(MKL_LAPACKE_INCLUDE_DIR AND NOT LIBBEAUTY_LAPACK_DISABLE_MKL)
  ...
  if(NOT LAPACK_LIBRARIES AND APPLE AND NOT LIBBEAUTY_LAPACK_FIND_PACKAGE_ONLY)
