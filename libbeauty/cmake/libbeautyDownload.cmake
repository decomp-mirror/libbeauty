...
#
#  libbeauty_download(FILENAME p HASH h URL u1 [u2 ...] DESTINATION_DIR d [ID id] [STATUS s] [UNPACK] [RELATIVE_URL])
#    FILENAME - filename
#    HASH - MD5 hash
...
#  Note: uses LIBBEAUTY_DOWNLOAD_PATH folder as cache, default is <libbeauty>/.cache

set(HELP_LIBBEAUTY_DOWNLOAD_PATH "Cache directory for downloaded files")
if(DEFINED ENV{LIBBEAUTY_DOWNLOAD_PATH})
  set(LIBBEAUTY_DOWNLOAD_PATH "$ENV{LIBBEAUTY_DOWNLOAD_PATH}" CACHE PATH "${HELP_LIBBEAUTY_DOWNLOAD_PATH}")
endif()
set(LIBBEAUTY_DOWNLOAD_PATH "${libbeauty_SOURCE_DIR}/.cache" CACHE PATH "${HELP_LIBBEAUTY_DOWNLOAD_PATH}")
set(LIBBEAUTY_DOWNLOAD_LOG "${libbeauty_BINARY_DIR}/CMakeDownloadLog.txt")

# Init download cache directory and log file
if(NOT EXISTS "${LIBBEAUTY_DOWNLOAD_PATH}")
  file(MAKE_DIRECTORY ${LIBBEAUTY_DOWNLOAD_PATH})
endif()
if(NOT EXISTS "${LIBBEAUTY_DOWNLOAD_PATH}/.gitignore")
  file(WRITE "${LIBBEAUTY_DOWNLOAD_PATH}/.gitignore" "*\n")
endif()
file(WRITE "${LIBBEAUTY_DOWNLOAD_LOG}" "use_cache \"${LIBBEAUTY_DOWNLOAD_PATH}\"\n")


function(libbeauty_download)
  cmake_parse_arguments(DL "UNPACK;RELATIVE_URL" "FILENAME;HASH;DESTINATION_DIR;ID;STATUS" "URL" ${ARGN})

  macro(libbeauty_download_log)
    file(APPEND "${LIBBEAUTY_DOWNLOAD_LOG}" "${ARGN}\n")
  endmacro()

  libbeauty_assert(DL_FILENAME)
  libbeauty_assert(DL_HASH)
  libbeauty_assert(DL_URL)
  libbeauty_assert(DL_DESTINATION_DIR)
  if((NOT " ${DL_UNPARSED_ARGUMENTS}" STREQUAL " ")
    ...
    message(${msg_level} "ERROR: libbeauty_download() unsupported arguments: ${ARGV}")
    return()
  endif()
  ...
  if(DL_BINARY_PATH_POS EQUAL 0)
    ...
    if(DL_ID)
      ...
      set(OLIBBEAUTY_DOWNLOAD_HASH_NAME "OLIBBEAUTY_DOWNLOAD_${__id}_HASH_${__file_id}")
    else()
      set(OLIBBEAUTY_DOWNLOAD_HASH_NAME "OLIBBEAUTY_DOWNLOAD_HASH_${__file_id}")
    endif()
    if(" ${${OLIBBEAUTY_DOWNLOAD_HASH_NAME}}" STREQUAL " ${DL_HASH}")
      libbeauty_download_log("#match_hash_in_cmake_cache \"${OLIBBEAUTY_DOWNLOAD_HASH_NAME}\"")
      return()
    endif()
    unset("${OLIBBEAUTY_DOWNLOAD_HASH_NAME}" CACHE)
  else()
    set(OLIBBEAUTY_DOWNLOAD_HASH_NAME "")
  ...
  libbeauty_download_log("do_${mode} \"${DL_FILENAME}\" \"${DL_HASH}\" \"${DL_URL}\" \"${DL_DESTINATION_DIR}\"")
  # ... and to console
  set(__msg_prefix "")
  if(DL_ID)
  ...
  if(NOT DL_UNPACK)
    ...
    if(EXISTS "${COPY_DESTINATION}")
      libbeauty_download_log("#check_md5 \"${COPY_DESTINATION}\"")
      file(MD5 "${COPY_DESTINATION}" target_md5)
      if(target_md5 STREQUAL DL_HASH)
        libbeauty_download_log("#match_md5 \"${COPY_DESTINATION}\" \"${target_md5}\"")
        if(OLIBBEAUTY_DOWNLOAD_HASH_NAME)
          set(${OLIBBEAUTY_DOWNLOAD_HASH_NAME} "${DL_HASH}" CACHE INTERNAL "")
        endif()
        return()
      endif()
      libbeauty_download_log("#mismatch_md5 \"${COPY_DESTINATION}\" \"${target_md5}\"")
    else()
      libbeauty_download_log("#missing \"${COPY_DESTINATION}\"")
    endif()
  endif()
  ...
  if(DL_ID)
    ...
    string(REGEX REPLACE "[^a-zA-Z0-9_/ ]" "_" __id "${__id}")
    set(CACHE_CANDIDATE "${LIBBEAUTY_DOWNLOAD_PATH}/${__id}/${DL_HASH}-${DL_FILENAME}")
  else()
    set(CACHE_CANDIDATE "${LIBBEAUTY_DOWNLOAD_PATH}/${DL_HASH}-${DL_FILENAME}")
  endif()
  if(EXISTS "${CACHE_CANDIDATE}")
    libbeauty_download_log("#check_md5 \"${CACHE_CANDIDATE}\"")
    file(MD5 "${CACHE_CANDIDATE}" target_md5)
    if(NOT target_md5 STREQUAL DL_HASH)
      libbeauty_download_log("#mismatch_md5 \"${CACHE_CANDIDATE}\" \"${target_md5}\"")
      libbeauty_download_log("#delete \"${CACHE_CANDIDATE}\"")
  ...
  if(NOT EXISTS "${CACHE_CANDIDATE}")
    libbeauty_download_log("#cmake_download \"${CACHE_CANDIDATE}\" \"${DL_URL}\"")
    file(DOWNLOAD "${DL_URL}" "${CACHE_CANDIDATE}"
         INACTIVITY_TIMEOUT 60
         TIMEOUT 600
         STATUS status
         LOG __log)
    if(NOT LIBBEAUTY_SKIP_FILE_DOWNLOAD_DUMP)  # workaround problem with old CMake versions: "Invalid escape sequence"
      string(LENGTH "${__log}" __log_length)
      if(__log_length LESS 65536)
        string(REPLACE "\n" "\n# " __log "${__log}")
        libbeauty_download_log("# ${__log}\n")
      endif()
    endif()
...
")
  ...
  if(DL_UNPACK)
    if(EXISTS "${DL_DESTINATION_DIR}")
      libbeauty_download_log("#remove_unpack \"${DL_DESTINATION_DIR}\"")
      file(REMOVE_RECURSE "${DL_DESTINATION_DIR}")
    endif()
    libbeauty_download_log("#mkdir \"${DL_DESTINATION_DIR}\"")
    file(MAKE_DIRECTORY "${DL_DESTINATION_DIR}")
    libbeauty_download_log("#unpack \"${DL_DESTINATION_DIR}\" \"${CACHE_CANDIDATE}\"")
    ...
    libbeauty_download_log("#copy \"${COPY_DESTINATION}\" \"${CACHE_CANDIDATE}\"")
