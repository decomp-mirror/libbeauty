...
# Debugging function
function(libbeauty_cmake_dump_vars)
...
#
option(LIBBEAUTY_DUMP_HOOKS_FLOW "Dump called libbeauty hooks" OFF)
macro(libbeauty_cmake_hook_append hook_name)
  set(__var_name "__LIBBEAUTY_CMAKE_HOOKS_${hook_name}")
...
endmacro()
macro(libbeauty_cmake_hook hook_name)
  set(__var_name "__LIBBEAUTY_CMAKE_HOOKS_${hook_name}")
  if(LIBBEAUTY_DUMP_HOOKS_FLOW)
  ...
  endforeach()
endmacro()
macro(libbeauty_cmake_reset_hooks)
  get_cmake_property(__variableNames VARIABLES)
  foreach(__variableName ${__variableNames})
    if(__variableName MATCHES "^__LIBBEAUTY_CMAKE_HOOKS_")
      unset(${__variableName})
      unset(${__variableName} CACHE)
    endif()
  endforeach()
endmacro()
macro(libbeauty_cmake_hook_register_dir dir)
  file(GLOB hook_files RELATIVE "${dir}" "${dir}/*.cmake")
  foreach(f ${hook_files})
    if(f MATCHES "^(.+)\\.cmake$")
      set(hook_name "${CMAKE_MATCH_1}")
      libbeauty_cmake_hook_append(${hook_name} "${dir}/${f}")
    endif()
  endforeach()
endmacro()


function(libbeauty_cmake_eval var_name)
  if(DEFINED ${var_name})
...
macro(libbeauty_cmake_configure file_name var_name)
  file(READ "${file_name}" __config)
  string(CONFIGURE "${__config}" ${var_name} ${ARGN})
endmacro()

macro(libbeauty_update VAR)
  if(NOT DEFINED ${VAR})
    ...
    #libbeauty_debug_message("Preserve old value for ${VAR}: ${${VAR}}")
  endif()
endmacro()
...
# Usage samples:
#   libbeauty_assert(MyLib_FOUND)
#   libbeauty_assert(DEFINED MyLib_INCLUDE_DIRS)
macro(libbeauty_assert)
  if(NOT (${ARGN}))
...
macro(libbeauty_debug_message)
#  string(REPLACE ";" " " __msg "${ARGN}")
#  message(STATUS "${__msg}")
endmacro()

macro(libbeauty_check_environment_variables)
  foreach(_var ${ARGN})
...
macro(libbeauty_path_join result_var P1 P2_)
...
macro(_libbeauty_fix_target target_var)
  if(BUILD_libbeauty_world)
    if(LIBBEAUTY_MODULE_${${target_var}}_IS_PART_OF_WORLD)
      set(${target_var} libbeauty_world)
    endif()
  endif()
endmacro()
...
function(libbeauty_is_subdir res dir sub )
...
function(libbeauty_is_libbeauty_directory result_var dir)
  set(result FALSE)
  foreach(parent ${libbeauty_SOURCE_DIR} ${libbeauty_BINARY_DIR} ${LIBBEAUTY_EXTRA_MODULES_PATH})
    libbeauty_is_subdir(result "${parent}" "${dir}")
    if(result)
      break()
    endif()
  endforeach()
...
# adds include directories in such a way that directories from the libbeauty source tree go first
function(libbeauty_include_directories)
  libbeauty_debug_message("libbeauty_include_directories( ${ARGN} )")
  set(__add_before "")
  foreach(dir ${ARGN})
    libbeauty_is_libbeauty_directory(__is_libbeauty_dir "${dir}")
    if(__is_libbeauty_dir)
...
function(libbeauty_append_target_property target prop)
...
function(libbeauty_append_dependant_targets target)
  #libbeauty_debug_message("libbeauty_append_dependant_targets(${target} ${ARGN})")
  _libbeauty_fix_target(target)
  set(LIBBEAUTY_DEPENDANT_TARGETS_${target} "${LIBBEAUTY_DEPENDANT_TARGETS_${target}};${ARGN}" CACHE INTERNAL "" FORCE)
endfunction()

# adds include directories in such a way that directories from the libbeauty source tree go first
function(libbeauty_target_include_directories target)
  #libbeauty_debug_message("libbeauty_target_include_directories(${target} ${ARGN})")
  _libbeauty_fix_target(target)
  set(__params "")
  ...
  if(HAVE_CUDA OR CMAKE_VERSION VERSION_LESS 2.8.11)
    ...
    if(TARGET ${target})
      if(__params)
        ...
        if(LIBBEAUTY_DEPENDANT_TARGETS_${target})
          foreach(t ${LIBBEAUTY_DEPENDANT_TARGETS_${target}})
      ...
      if(__params)
        set(__new_inc ${OLIBBEAUTY_TARGET_INCLUDE_DIRS_${target}})
        list(APPEND __new_inc ${__params})
        set(OLIBBEAUTY_TARGET_INCLUDE_DIRS_${target} "${__new_inc}" CACHE INTERNAL "")
      endif()
      if(__system_params)
        set(__new_inc ${OLIBBEAUTY_TARGET_INCLUDE_SYSTEM_DIRS_${target}})
        list(APPEND __new_inc ${__system_params})
        set(OLIBBEAUTY_TARGET_INCLUDE_SYSTEM_DIRS_${target} "${__new_inc}" CACHE INTERNAL "")
      endif()
    endif()
  endif()
endfunction()
...
set(OLIBBEAUTY_COMPILER_FAIL_REGEX
...
MACRO(libbeauty_check_compiler_flag LANG FLAG RESULT)
  set(_fname "${ARGN}")
  if(NOT DEFINED ${RESULT})
    ...
    if(_fname)
      ...
      if(${RESULT})
        ...
        foreach(_regex ${OLIBBEAUTY_COMPILER_FAIL_REGEX})
          if(NOT ${RESULT})
            break()
          endif()
...
macro(libbeauty_check_flag_support lang flag varname base_options)
  if(CMAKE_BUILD_TYPE)
  ...
  libbeauty_check_compiler_flag("${_lang}" "${base_options} ${flag}" ${${varname}} ${ARGN})
endmacro()

# turns off warnings
macro(libbeauty_warnings_disable)
  if(NOT ENABLE_NOISY_WARNINGS)
    ...
    if(MSVC AND _msvc_warnings AND _flag_vars)
      foreach(var ${_flag_vars})
        ...
        endforeach()
      endforeach()
    elseif((CMAKE_COMPILER_IS_GNUCXX OR (UNIX AND LIBBEAUTY_ICC)) AND _gxx_warnings AND _flag_vars)
      foreach(var ${_flag_vars})
        foreach(warning ${_gxx_warnings})
          ...
          libbeauty_check_flag_support(${var} "${warning}" _varname "")
          if(${_varname})
            set(${var} "${${var}} ${warning}")
          endif()
        endforeach()
    ...
    if(LIBBEAUTY_ICC AND _icc_warnings AND _flag_vars)
      foreach(var ${_flag_vars})
...
macro(libbeauty_append_source_file_compile_definitions source)
...
macro(add_apple_compiler_options the_module)
  libbeauty_check_flag_support(OBJCXX "-fobjc-exceptions" HAVE_OBJC_EXCEPTIONS "")
  if(HAVE_OBJC_EXCEPTIONS)
    foreach(source ${LIBBEAUTY_MODULE_${the_module}_SOURCES})
...
macro(OLIBBEAUTY_OPTION variable description value)
  set(__value ${value})
  set(__condition "")
...
# Usage: libbeauty_append_build_options(HIGHGUI FFMPEG)
macro(libbeauty_append_build_options var_prefix pkg_prefix)
...
macro(libbeauty_check_modules define)
  unset(HAVE_${define})
  foreach(m ${ARGN})
    if (m MATCHES "(.*[^><])(>=|=|<=)(.*)")
...
macro(CHECK_MODULE module_name define cv_module)
  ...
  if(PKG_CONFIG_FOUND)
    ...
    if(${ALIAS_FOUND})
      set(${define} 1)
      libbeauty_append_build_options(${cv_module} ${ALIAS})
    endif()
  endif()
endmacro()
...
OLIBBEAUTY_OPTION(BUILD_USE_SYMLINKS "Use symlinks instead of files copying during build (and !!INSTALL!!)" (${__symlink_default}) IF (UNIX OR DEFINED __symlink_default))

if(CMAKE_VERSION VERSION_LESS "3.2")
  macro(libbeauty_cmake_byproducts var_name)
...
set(LIBBEAUTY_DEPHELPER "${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/dephelper" CACHE INTERNAL "")
file(MAKE_DIRECTORY ${LIBBEAUTY_DEPHELPER})
...
set(LIBBEAUTY_BUILD_INFO_STR "" CACHE INTERNAL "")
function(libbeauty_output_status msg)
  message(STATUS "${msg}")
  string(REPLACE "\\" "\\\\" msg "${msg}")
  string(REPLACE "\"" "\\\"" msg "${msg}")
  set(LIBBEAUTY_BUILD_INFO_STR "${LIBBEAUTY_BUILD_INFO_STR}\"${msg}\\n\"\n" CACHE INTERNAL "")
endfunction()

macro(libbeauty_finalize_status)
  set(LIBBEAUTY_BUILD_INFO_FILE "${CMAKE_BINARY_DIR}/version_string.tmp")
  if(EXISTS "${LIBBEAUTY_BUILD_INFO_FILE}")
    file(READ "${LIBBEAUTY_BUILD_INFO_FILE}" __content)
  else()
    set(__content "")
  endif()
  if("${__content}" STREQUAL "${LIBBEAUTY_BUILD_INFO_STR}")
    #message(STATUS "${LIBBEAUTY_BUILD_INFO_FILE} contains the same content")
  else()
    file(WRITE "${LIBBEAUTY_BUILD_INFO_FILE}" "${LIBBEAUTY_BUILD_INFO_STR}")
  endif()
  unset(__content)
  unset(LIBBEAUTY_BUILD_INFO_STR CACHE)

  if(NOT LIBBEAUTY_SKIP_STATUS_FINALIZATION)
    if(DEFINED LIBBEAUTY_MODULE_libbeauty_core_BINARY_DIR)
      execute_process(COMMAND ${CMAKE_COMMAND} -E copy_if_different "${LIBBEAUTY_BUILD_INFO_FILE}" "${LIBBEAUTY_MODULE_libbeauty_core_BINARY_DIR}/version_string.inc" OUTPUT_QUIET)
    endif()
  endif()
  ...
  if(DEFINED status_cond)
    ...
    if(DEFINED status_then OR DEFINED status_else)
      if(${status_cond})
        ...
        libbeauty_output_status("${status_text} ${status_then}")
      else()
        string(REPLACE ";" " " status_else "${status_else}")
        string(REGEX REPLACE "^[ \t]+" "" status_else "${status_else}")
        libbeauty_output_status("${status_text} ${status_else}")
      endif()
    else()
      string(REPLACE ";" " " status_cond "${status_cond}")
      string(REGEX REPLACE "^[ \t]+" "" status_cond "${status_cond}")
      libbeauty_output_status("${status_text} ${status_cond}")
    endif()
  else()
    libbeauty_output_status("${text}")
  endif()
...
#
# Usage:
#   libbeauty_build_features_string(out [EXCLUSIVE] [IF feature THEN title] ... [ELSE title])
#
function(libbeauty_build_features_string out)
  set(result)
...
macro(libbeauty_list_filterout lst regex)
  foreach(item ${${lst}})
...
macro(libbeauty_list_filter lst regex)
  set(dst ${ARGN})
  if(NOT dst)
    set(dst ${lst})
  endif()
...
macro(libbeauty_list_unique __lst)
  if(${__lst})
...
macro(libbeauty_list_reverse __lst)
  if(${__lst})
    list(REVERSE ${__lst})
  endif()
endmacro()
...
macro(libbeauty_list_sort __lst)
  if(${__lst})
    list(SORT ${__lst})
  endif()
endmacro()
...
macro(libbeauty_list_add_prefix LST PREFIX)
  set(__tmp "")
  foreach(item ${${LST}})
...
macro(libbeauty_list_add_suffix LST SUFFIX)
  set(__tmp "")
  foreach(item ${${LST}})
...
macro(libbeauty_list_pop_front LST VAR)
  if(${LST})
    list(GET ${LST} 0 ${VAR})
...
# libbeauty_get_duplicates(<output list> <element> [<element> ...])
function(libbeauty_get_duplicates res)
  if(ARGC LESS 2)
    message(FATAL_ERROR "Invalid call to libbeauty_get_duplicates")
  endif()
  set(lst ${ARGN})
  list(SORT lst)
  set(prev_item)
...
macro(libbeauty_regex_escape var regex)
  string(REGEX REPLACE "([+.*^$])" "\\\\1" ${var} "${regex}")
endmacro()


# convert list of paths to full paths
macro(libbeauty_convert_to_full_paths VAR)
  if(${VAR})
    set(__tmp "")
    foreach(path ${${VAR}})
...
function(libbeauty_convert_to_lib_name var)
  set(tmp "")
  foreach(path ${ARGN})
    get_filename_component(tmp_name "${path}" NAME)
    libbeauty_get_libname(tmp_name "${tmp_name}")
...
# add install command
function(libbeauty_install_target)
  if(APPLE_FRAMEWORK AND BUILD_SHARED_LIBS)
    install(TARGETS ${ARGN} FRAMEWORK DESTINATION ${LIBBEAUTY_3P_LIB_INSTALL_PATH})
  else()
...
macro(libbeauty_parse_header FILENAME FILE_VAR)
  set(vars_regex "")
...
macro(libbeauty_parse_header2 LIBNAME HDR_PATH VARNAME)
  libbeauty_clear_vars(${LIBNAME}_VERSION_MAJOR
...
macro(libbeauty_parse_pkg LIBNAME PKG_PATH SCOPE)
...
function(libbeauty_source_group group)
  if(BUILD_libbeauty_world AND LIBBEAUTY_MODULE_${the_module}_IS_PART_OF_WORLD)
...
macro(__libbeauty_push_target_link_libraries)
  if(NOT TARGET ${target})
    if(NOT DEFINED LIBBEAUTY_MODULE_${target}_LOCATION)
      message(FATAL_ERROR "libbeauty_target_link_libraries: invalid target: '${target}'")
    endif()
    set(LIBBEAUTY_MODULE_${target}_LINK_DEPS ${LIBBEAUTY_MODULE_${target}_LINK_DEPS} ${ARGN} CACHE INTERNAL "" FORCE)
  else()
    target_link_libraries(${target} ${ARGN})
  endif()
endmacro()

function(libbeauty_target_link_libraries target)
  set(LINK_DEPS ${ARGN})
  _libbeauty_fix_target(target)
  ...
  foreach(dep ${LINK_DEPS})
    ...
    elseif(" ${dep}" STREQUAL " LINK_PRIVATE" OR " ${dep}" STREQUAL "LINK_PUBLIC")
      if(NOT LINK_PENDING STREQUAL "")
        __libbeauty_push_target_link_libraries(${LINK_MODE} ${LINK_PENDING})
        set(LINK_PENDING "")
        set(LINK_MODE "${dep}")
      endif()
    else()
      if(BUILD_libbeauty_world)
        if(LIBBEAUTY_MODULE_${dep}_IS_PART_OF_WORLD)
          set(dep libbeauty_world)
        endif()
      endif()
...
function(libbeauty_target_compile_definitions target)
  _libbeauty_fix_target(target)
  if(NOT TARGET ${target})
    if(NOT DEFINED LIBBEAUTY_MODULE_${target}_LOCATION)
      message(FATAL_ERROR "libbeauty_target_compile_definitions: invalid target: '${target}'")
    endif()
    set(LIBBEAUTY_MODULE_${target}_COMPILE_DEFINITIONS ${LIBBEAUTY_MODULE_${target}_COMPILE_DEFINITIONS} ${ARGN} CACHE INTERNAL "" FORCE)
  else()
...
function(_libbeauty_append_target_includes target)
  if(DEFINED OLIBBEAUTY_TARGET_INCLUDE_DIRS_${target})
    target_include_directories(${target} PRIVATE ${OLIBBEAUTY_TARGET_INCLUDE_DIRS_${target}})
    if(LIBBEAUTY_DEPENDANT_TARGETS_${target})
      foreach(t ${LIBBEAUTY_DEPENDANT_TARGETS_${target}})
        target_include_directories(${t} PRIVATE ${OLIBBEAUTY_TARGET_INCLUDE_DIRS_${target}})
      endforeach()
    endif()
    unset(OLIBBEAUTY_TARGET_INCLUDE_DIRS_${target} CACHE)
  endif()

  if(DEFINED OLIBBEAUTY_TARGET_INCLUDE_SYSTEM_DIRS_${target})
    target_include_directories(${target} SYSTEM PRIVATE ${OLIBBEAUTY_TARGET_INCLUDE_SYSTEM_DIRS_${target}})
    if(LIBBEAUTY_DEPENDANT_TARGETS_${target})
      foreach(t ${LIBBEAUTY_DEPENDANT_TARGETS_${target}})
        target_include_directories(${t} SYSTEM PRIVATE ${OLIBBEAUTY_TARGET_INCLUDE_SYSTEM_DIRS_${target}})
      endforeach()
    endif()
    unset(OLIBBEAUTY_TARGET_INCLUDE_SYSTEM_DIRS_${target} CACHE)
  endif()
endfunction()

function(libbeauty_add_executable target)
  add_executable(${target} ${ARGN})
  _libbeauty_append_target_includes(${target})
endfunction()

function(libbeauty_add_library target)
  if(HAVE_CUDA AND ARGN MATCHES "\\.cu")
    libbeauty_include_directories(${CUDA_INCLUDE_DIRS})
    libbeauty_cuda_compile(cuda_objs ${ARGN})
    set(LIBBEAUTY_MODULE_${target}_CUDA_OBJECTS ${cuda_objs} CACHE INTERNAL "Compiled CUDA object files")
  endif()
  ...
  if(APPLE_FRAMEWORK AND BUILD_SHARED_LIBS)
    ...
    set_target_properties(${target} PROPERTIES
      FRAMEWORK TRUE
      MACOSX_FRAMEWORK_IDENTIFIER org.libbeauty
      MACOSX_FRAMEWORK_INFO_PLIST ${CMAKE_BINARY_DIR}/ios/Info.plist
      # "current version" in semantic format in Mach-O binary file
      VERSION ${LIBBEAUTY_LIBVERSION}
      # "compatibility version" in semantic format in Mach-O binary file
      SOVERSION ${LIBBEAUTY_LIBVERSION}
      INSTALL_RPATH ""
      INSTALL_NAME_DIR "@rpath"
      BUILD_WITH_INSTALL_RPATH 1
      LIBRARY_OUTPUT_NAME "libbeauty"
      XCODE_ATTRIBUTE_TARGETED_DEVICE_FAMILY "1,2"
      #PUBLIC_HEADER "${LIBBEAUTY_CONFIG_FILE_INCLUDE_DIR}/libbeautyconfig.h"
...
macro(libbeauty_get_libname var_name)
  get_filename_component(__libname "${ARGN}" NAME)
  # liblibbeauty_core.so.3.3 -> libbeauty_core
  string(REGEX REPLACE "^lib(.+)\\.(a|so|dll)(\\.[.0-9]+)?$" "\\1" __libname "${__libname}")
  # MacOSX: liblibbeauty_core.3.3.1.dylib -> libbeauty_core
...
macro(libbeauty_get_all_libs _modules _extra _3rdparty)
  set(${_modules} "")
  set(${_extra} "")
  set(${_3rdparty} "")
  foreach(m ${LIBBEAUTY_MODULES_PUBLIC})
    if(TARGET ${m})
    ...
    libbeauty_list_reverse(_rev_deps)
    foreach (dep ${_rev_deps})
      if(DEFINED LIBBEAUTY_MODULE_${dep}_LOCATION)
        list(INSERT ${_modules} 0 ${dep})
      endif()
    endforeach()
    foreach (dep ${deps} ${LIBBEAUTY_LINKER_LIBS})
      if (NOT DEFINED LIBBEAUTY_MODULE_${dep}_LOCATION)
        ...
        if (TARGET ${dep})
          ...
          if(_type STREQUAL "STATIC_LIBRARY" AND BUILD_SHARED_LIBS OR _type STREQUAL "INTERFACE_LIBRARY")
            ...
            if (_POS EQUAL 0)
              libbeauty_get_libname(_libname "${_output_name}")
  ...
  libbeauty_list_filterout(${_modules} "^[\$]<")
  libbeauty_list_filterout(${_3rdparty} "^[\$]<")
  libbeauty_list_filterout(${_extra} "^[\$]<")

  # convert CMake lists to makefile literals
  foreach(lst ${_modules} ${_3rdparty} ${_extra})
    libbeauty_list_unique(${lst})
    libbeauty_list_reverse(${lst})
  endforeach()
endmacro()


function(libbeauty_add_test_from_target test_name test_kind the_target)
  if(CMAKE_VERSION VERSION_GREATER "2.8" AND NOT CMAKE_CROSSCOMPILING)
    ...
    set_tests_properties("${test_name}" PROPERTIES
      LABELS "${LIBBEAUTY_MODULE_${the_module}_LABEL};${test_kind}"
      WORKING_DIRECTORY "${test_report_dir}")

    if(LIBBEAUTY_TEST_DATA_PATH)
      set_tests_properties("${test_name}" PROPERTIES
        ENVIRONMENT "LIBBEAUTY_TEST_DATA_PATH=${LIBBEAUTY_TEST_DATA_PATH}")
    endif()
  endif()
endfunction()

macro(libbeauty_add_testdata basedir dest_subdir)
  if(BUILD_TESTS)
    if(NOT CMAKE_CROSSCOMPILING AND NOT INSTALL_TESTS)
      file(COPY ${basedir}/
           DESTINATION ${CMAKE_BINARY_DIR}/${LIBBEAUTY_TEST_DATA_INSTALL_PATH}/${dest_subdir}
           ${ARGN}
      )
    endif()
    if(INSTALL_TESTS)
      install(DIRECTORY ${basedir}/
              DESTINATION ${LIBBEAUTY_TEST_DATA_INSTALL_PATH}/${dest_subdir}
              COMPONENT "tests"
              ${ARGN}
      )
    endif()
...
macro(libbeauty_generate_vs_version_file DESTINATION)
  ...
  macro(__vs_ver_update_variable name)
    if(VS_VER_NAME AND DEFINED LIBBEAUTY_${VS_VER_NAME}_VS_VER_${name})
      set(LIBBEAUTY_VS_VER_${name} "${LIBBEAUTY_${VS_VER_NAME}_VS_VER_${name}}")
    elseif(VS_VER_${name})
      set(LIBBEAUTY_VS_VER_${name} "${VS_VER_${name}}")
    endif()
  endmacro()
  ...
  macro(__vs_ver_update_str_variable name)
    if(VS_VER_NAME AND DEFINED LIBBEAUTY_${VS_VER_NAME}_VS_VER_${name})
      set(LIBBEAUTY_VS_VER_${name}_STR "${LIBBEAUTY_${VS_VER_NAME}_VS_VER_${name}}")
    elseif(VS_VER_${name})
      set(LIBBEAUTY_VS_VER_${name}_STR "${VS_VER_${name}}")
    endif()
  endmacro()
  ...
  if(LIBBEAUTY_VS_VER_COPYRIGHT_STR)
    set(LIBBEAUTY_VS_VER_HAVE_COPYRIGHT_STR 1)
  else()
    set(LIBBEAUTY_VS_VER_HAVE_COPYRIGHT_STR 0)
  endif()

  if(LIBBEAUTY_VS_VER_COMMENTS_STR)
    set(LIBBEAUTY_VS_VER_HAVE_COMMENTS_STR 1)
  else()
    set(LIBBEAUTY_VS_VER_HAVE_COMMENTS_STR 0)
  endif()

  configure_file("${libbeauty_SOURCE_DIR}/cmake/templates/vs_version.rc.in" "${DESTINATION}" @ONLY)
endmacro()

macro(libbeauty_cmake_script_append_var content_var)
...
macro(libbeauty_copyfiles_append_file list_var src dst)
...
macro(libbeauty_copyfiles_append_dir list_var src dst)
  set(__glob ${ARGN})
  list(LENGTH ${list_var} __id)
  list(APPEND ${list_var} ${__id})
  set(${list_var}_SRC_${__id} "${src}")
...
macro(libbeauty_copyfiles_make_config_string content_var list_var)
...
macro(libbeauty_copyfiles_make_config_file filename_var list_var)
  libbeauty_copyfiles_make_config_string(${list_var}_CONFIG ${list_var})
  set(${filename_var} "${CMAKE_CURRENT_BINARY_DIR}/copyfiles-${list_var}.cmake")
  file(WRITE "${${filename_var}}" "${${list_var}_CONFIG}")
endmacro()

macro(libbeauty_copyfiles_add_forced_target target list_var comment_str)
  libbeauty_copyfiles_make_config_file(CONFIG_FILE ${list_var})
  libbeauty_cmake_byproducts(__byproducts BYPRODUCTS "${LIBBEAUTY_DEPHELPER}/${target}")
  add_custom_target(${target}
      ${__byproducts}  # required for add_custom_target() by ninja
      COMMAND ${CMAKE_COMMAND}
        "-DCONFIG_FILE:PATH=${CONFIG_FILE}"
        "-DCOPYLIST_VAR:STRING=${list_var}"
        "-DDEPHELPER=${LIBBEAUTY_DEPHELPER}/${target}"
        -P "${libbeauty_SOURCE_DIR}/cmake/copy_files.cmake"
      WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
      COMMENT "${comment_str}"
      DEPENDS "${libbeauty_SOURCE_DIR}/cmake/copy_files.cmake"
              # ninja warn about file(WRITE): "${SRC_COPY_CONFIG_FILE}"
  )
endmacro()

macro(libbeauty_copyfiles_add_target target list_var comment_str)
  set(deps ${ARGN})
  libbeauty_copyfiles_make_config_file(CONFIG_FILE ${list_var})
  add_custom_command(OUTPUT "${LIBBEAUTY_DEPHELPER}/${target}"
      COMMAND ${CMAKE_COMMAND}
        "-DCONFIG_FILE:PATH=${CONFIG_FILE}"
        "-DCOPYLIST_VAR:STRING=${list_var}"
        "-DDEPHELPER=${LIBBEAUTY_DEPHELPER}/${target}"
      ...
      WORKING_DIRECTORY ${CMAKE_BINARY_DIR}
      COMMENT "${comment_str}"
      DEPENDS "${libbeauty_SOURCE_DIR}/cmake/copy_files.cmake" ${deps}
              # ninja warn about file(WRITE): "${SRC_COPY_CONFIG_FILE}"
  )
  add_custom_target(${target} DEPENDS "${LIBBEAUTY_DEPHELPER}/${target}")
endmacro()

macro(libbeauty_get_smart_file_name output_var fpath)
  libbeauty_is_subdir(__subir "${libbeauty_BINARY_DIR}" "${fpath}")
  if(__subir)
    file(RELATIVE_PATH ${output_var} "${libbeauty_BINARY_DIR}" "${fpath}")
    set(${output_var} "<BUILD>/${${output_var}}")
  else()
    libbeauty_is_subdir(__subir "${libbeauty_SOURCE_DIR}" "${fpath}")
    if(__subir)
      file(RELATIVE_PATH ${output_var} "${libbeauty_SOURCE_DIR}" "${fpath}")
    else()
